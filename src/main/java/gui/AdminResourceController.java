...
public class AdminResourceController {
    ...
    private void openQuizManagementModal(Ressource res) {
        try {
    ...
    private void openTutorModal() {
        try {
            ...
            // Remove blur when dialog is hidden
            stage.setOnHidden(e -> rootNode.setEffect(null));
            
            // Set window transparency/rounded corners if possible (optional)
            stage.initStyle(javafx.stage.StageStyle.TRANSPARENT);
            root.setStyle("-fx-background-color: transparent;");
    ...
    private void closeTutorModal() {
        // This is now handled by the standalone dialog, 
        // but we keep the method to avoid FXML errors if it's still linked.
    }
